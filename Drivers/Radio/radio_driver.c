...
#define XTAL_DEFAULT_CAP_VALUE      ( 0x08UL ) //Default: XTAL_DEFAULT_CAP_VALUE      ( 0x20UL )
...
void SUBGRF_Init( DioIrqHandler dioIrq )
{
    ...
    {
        SUBGRF_WriteRegister( REG_XTA_TRIM, XTAL_DEFAULT_CAP_VALUE);//XTA_TRIM = 0x0911       XTB_TRIM = 0x0912    Default : XTAL_DEFAULT_CAP_VALUE
        SUBGRF_WriteRegister( REG_XTB_TRIM, XTAL_DEFAULT_CAP_VALUE);//TODO modify values to match frequency :)
//With 0x08UL 868.00075 MHz
        //With 0x09UL 867.99875 MHz
...
void SUBGRF_SetTxParams( uint8_t paSelect, int8_t power, RadioRampTimes_t rampTime )
{
    ...
    else /* rfo_hp*/
    {
    	...
    	//SUBGRF_WriteRegister(REG_TX_CLAMP, SUBGRF_ReadRegister(REG_TX_CLAMP) | 0x04 );
        /* WORKAROUND END*/
        ...
        SUBGRF_WriteRegister(REG_OCP, 0x38);   /*0x38  urrent max 160mA for the whole device*/

        //ADDED BY MARTIN
        SUBGRF_WriteRegister(SUBGHZ_PCR, 0b01110000);   //max 200mA /*Sub-GHz radio power control register*/
    }
    buf[0] = power;
...
uint8_t SUBGRF_SetRfTxPower( int8_t power )
{
    ...
    SUBGRF_SetTxParams( paSelect, power, RADIO_RAMP_40_US);//RADIO_RAMP_40_US );
